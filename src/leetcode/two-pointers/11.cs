...
{
    int s = 0, l = height.Length - 1;

    int output = 0;

    while (s < l)
    {
        output = Math.Max(output, Math.Min(height[s], height[l]) * (l - s));
...
lines that together with the x-axis form a container, such that the container contains 
